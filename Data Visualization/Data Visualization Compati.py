...
# Params
number_hero = 10
standard: Literal['percentage', 'softmax'] = 'none'
...
compatibility_dict = {key: [] for key in top_hero_list}
for df in df_list:
    for hero in list(compatibility_dict.keys()):
        compatibility_dict[hero].append(df[df['Unnamed: 4'] == hero]['Total friendship'].iloc[0])
...
plt.ylabel('Total friendship')
plt.title(f'Top {number_hero} Compati')
