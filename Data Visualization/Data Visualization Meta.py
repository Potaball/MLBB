...
# Extract metability value of top heroes
metability_dict = {key: [] for key in top_hero_list}
for df in df_list:
    for hero in list(metability_dict.keys()):
        metability_dict[hero].append(df[df['Hero'] == hero]['Metability'].iloc[0])


# Filter positive value only
x = np.arange(0,len(df_list))
y = np.array(list(metability_dict.values()))
labels = np.array(list(metability_dict.keys()))
