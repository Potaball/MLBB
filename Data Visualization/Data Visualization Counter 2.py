...
for df in df_list:
    ...
    enemy_list += [name[0] for name in ast.literal_eval(df['Counter by'].iloc[hero_index])]
enemy_list = list(set(enemy_list))


# Extract counteraility value of friends
counterability_dict = {key: [] for key in enemy_list}
for df in df_list:
    hero_index = df[df['Unnamed: 4'] == my_hero].index.tolist()[0]
    extracted_value = ast.literal_eval(df['Counter by'].iloc[hero_index])
    for name in extracted_value:
        if name[0] in list(counterability_dict.keys()):
            counterability_dict[name[0]].append(name[1])
    for hero in list(counterability_dict.keys()):
        if hero not in [name[0] for name in extracted_value]:
            counterability_dict[hero].append(0)
...
# Visualize the data
legend_ncol = math.ceil(len(enemy_list) / 25)  
...
plt.ylabel('Counter Percentage')
plt.title(f'Counter of {my_hero}')
plt.show()